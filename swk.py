...
╔   ─────━ ░ NEVER ░ ━─────   ╗
 ⌦ [ Gmail ] 
     ...
     ✺ 𝚛𝚎𝚜𝚝 : {InstagramScraper.get_rest(username)}
 ◢─────━ ░ NEVER ░━──────◣
        ❝ By @Nvvvvvvr ❞
    '''
        except:
...
class AccountValidator:
    ...
    def display_status(self, current_email: str = ""):
        ...
        print(f"""
{Colors.O}+------------------------------------------+
| {Colors.R}[1]{Colors.F} Hits ==> {Colors.F}[ {self.hits} ] 
| {Colors.B}[2]{Colors.R} Bad  ==> {Colors.R}[ {self.bads_instagram} ] 
| {Colors.B}[3]{Colors.X} Email Not Avail.==> {Colors.B}[ {self.bads_email} ]
| {Colors.B}[4]{Colors.X} Email  ==> {Colors.B}[ {current_email} ]   
| {Colors.B}By: @TTT9KK | @Nvvvvvvr{Colors.B}                    
{Colors.C}+------------------------------------------+
""")
